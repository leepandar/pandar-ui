  ...
  <a-card :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <a-row class="w-full h-full flex flex-col overflow-x-auto overflow-y-hidden">
      ...
      <a-row class="w-full flex justify-between">
        <a-space>
          <!-- 添加 -->
          <a-button v-perm="['sys:config:add']" type="primary" size="small" @click="addBtnClick()">
            <template #icon>
              <icon-plus/>
            </template>
      ...
      <a-row class="w-full flex-1 mt-3 overflow-y-auto">
        <a-table class="w-[100%]" :scroll="{ minWidth: 600, y: '100%' }" :columns="datatable.columns"
          ...
          <template #operation="{ record }">
            <a-button v-perm="['sys:config:edit']" type="text" size="mini" @click="updateBtnClick(record.configId)">
              <template #icon>
                <icon-edit/>
              </template>
              <template #default>修改</template>
            </a-button>
            <a-popconfirm v-perm="['sys:config:del']" content="确认要删除吗?" @ok="deleteBtnOkClick(record.configId)">
    ...
    <!-- 添加/修改 -->
    <a-modal v-model:visible="modal.visible" :width="modal.width" :esc-to-close="false" :mask-closable="false" draggable
             :footer="false">
...
import ConfigEdit from "~/pages/sys/config/ConfigEdit.vue";
import ConfigDetail from "~/pages/sys/config/ConfigDetail.vue";
