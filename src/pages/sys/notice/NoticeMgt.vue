  ...
  <a-card class="p-0" :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <a-row class="w-full h-full flex flex-col overflow-x-auto overflow-y-hidden">
        ...
        <a-form :model="searchForm" layout="inline" label-align="left" size="small">
          ...
          <a-form-item field="noticeType" label="公告类型">
            <a-select v-model="searchForm.noticeType" placeholder="公告类型" allow-clear>
              <a-option v-for="(d, index) in dicts[proxy.DICT.noticeType]" :key="index" :value="d.dictKey"
                        :label="d.dictValue"/>
            </a-select>
          </a-form-item>
      ...
      <a-row class="w-full flex justify-between">
        <a-space>
          <!-- 添加 -->
          <a-button v-perm="['sys:notice:add']" type="primary" size="small" @click="addBtnClick()">
            <template #icon>
              <icon-plus/>
            </template>
      ...
      <a-row class="w-full flex-1 mt-3 overflow-y-auto">
        <a-table class="w-[100%]" :scroll="{ minWidth: 600, y: '100%' }" :columns="datatable.columns"
          ...
          <template #operation="{ record }">
            <a-button v-perm="['sys:notice:edit']" type="text" size="mini" @click="updateBtnClick(record.noticeId)">
              <template #icon>
                <icon-edit/>
              </template>
            ...
            <a-popconfirm content="确认要删除吗?" @ok="deleteBtnOkClick(record.noticeId)">
              <a-button v-perm="['sys:notice:del']" type="text" status="danger" size="mini">
                <template #icon>
                  <icon-delete/>
                </template>
    ...
    <!-- 添加/修改 -->
    <a-modal v-model:visible="modal.visible" fullscreen :esc-to-close="false" :mask-closable="false" draggable
             :footer="false">
...
import NoticeEdit from "~/pages/sys/notice/NoticeEdit.vue";
import NoticeDetail from "~/pages/sys/notice/NoticeDetail.vue";
//全局实例
...
const datatable = reactive({
  ...
  columns: [
    ...
    {
      title: '发布部门',
      dataIndex: 'publishDeptId',
      slotName: 'publishDeptId',
      align: 'center',
      width: 150,
      ellipsis: true,
      tooltip: true
