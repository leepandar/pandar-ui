  ...
  <a-card :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <a-row class="w-full h-full flex flex-col overflow-x-auto overflow-y-hidden">
      ...
      <a-row class="w-full flex justify-between">
        <a-space>
          <!-- 添加 -->
          <a-button v-perm="['sys:dept:add']" type="primary" size="small" @click="addBtnClick()">
            <template #icon>
              <icon-plus/>
            </template>
      ...
      <a-row class="w-full flex-1 mt-3 overflow-y-auto">
        <a-table ref="tableRef" class="w-[100%]" :scroll="{y: '100%'}" :columns="datatable.columns"
                 :data="datatable.records" :loading="datatable.loading" row-key="deptId" :pagination="false"
                 table-layout-fixed>
          <!-- 部门名称 -->
          ...
          <template #operation="{ record }">
            <a-button v-perm="['sys:dept:add']" type="text" size="mini" @click="addRowBtnClick(record)"
                      style="padding: 0 5px">
              <template #icon>
                <icon-plus/>
              </template>
            ...
            <a-button v-perm="['sys:dept:edit']" type="text" size="mini" @click="updateBtnClick(record.deptId)"
                      style="padding: 0 5px">
              <template #icon>
                <icon-edit/>
              </template>
            ...
            <a-popconfirm content="确认要删除吗?" @ok="deleteBtnOkClick(record)">
              <a-button v-perm="['sys:dept:del']" type="text" status="danger" size="mini" style="padding: 0 5px">
                <template #icon>
                  <icon-delete/>
                </template>
...
import DeptEdit from "~/pages/sys/dept/DeptEdit.vue";
import DeptDetail from "~/pages/sys/dept/DeptDetail.vue";
