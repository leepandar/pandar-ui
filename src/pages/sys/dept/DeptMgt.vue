            ...
            <a-row class="w-full flex-1 mt-3 overflow-y-auto">
                <a-table ref="tableRef" class="w-[100%]" :scroll="{y: '100%'}" :columns="datatable.columns" :data="datatable.records" :loading="datatable.loading" row-key="deptId" :pagination="false" table-layout-fixed>
                    ...
                    <template #operation="{ record }">
                        <a-button v-perm="['sys:dept:add']" type="text" size="mini" @click="addRowBtnClick(record)" style="padding: 0 5px">
                            <template #icon>
                                <icon-plus />
                            </template>
                            <template #default>添加</template>
                        </a-button>
                        <a-button v-perm="['sys:dept:edit']" type="text" size="mini" @click="updateBtnClick(record.deptId)" style="padding: 0 5px">
                            <template #icon>
                                <icon-edit />
                            </template>
