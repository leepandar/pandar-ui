  ...
  <a-card :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <a-row class="w-full h-full flex flex-col overflow-x-auto overflow-y-hidden">
      ...
      <a-row class="w-full flex justify-between">
        <a-space>
          <!-- 添加 -->
          <a-button v-perm="['sys:tenant:add']" type="primary" size="small" @click="addBtnClick()">
            <template #icon>
              <icon-plus/>
            </template>
      ...
      <a-row class="w-full flex-1 mt-3 overflow-y-auto">
        <a-table class="w-[100%]" :scroll="{ minWidth: 600, y: '100%' }" :columns="datatable.columns"
                 :data="datatable.records" :loading="datatable.loading" :pagination="false" table-layout-fixed>
          <!-- 租户名 -->
          ...
          <template #packageId="{ record }">
            <dict-convert :dict-data="dicts[proxy.DICT.tenantPackageList]"
                          :dict-key="record.packageId === '0' ? Number(record.packageId) : String(record.packageId)"/>
          </template>
          <!-- 数据隔离方式 -->
          ...
          <template #operation="{ record }">
            <a-button v-perm="['sys:tenant:edit']" type="text" size="mini" @click="updateBtnClick(record.tenantId)">
              <template #icon>
                <icon-edit/>
              </template>
              <template #default>修改</template>
            </a-button>
            <a-popconfirm content="删除租户会导致租户下的所有用户无法使用，确认要删除吗?"
                          @ok="deleteBtnOkClick(record.tenantId)">
              <a-button v-perm="['sys:tenant:del']" type="text" status="danger" size="mini">
                <template #icon>
                  <icon-delete/>
                </template>
    ...
    <!-- 添加/修改 -->
    <a-modal v-model:visible="modal.visible" width="50%" :esc-to-close="false" :mask-closable="false" draggable
             :footer="false">
...
import TenantDetail from "~/pages/sys/tenant/TenantDetail.vue";
import TenantEdit from "~/pages/sys/tenant/TenantEdit.vue";
