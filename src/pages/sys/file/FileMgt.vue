<template>
  <a-card :body-style="{height: 'calc(100vh - 125px)'}">
      ...
      <a-row class="h-full border-r pr-3">
        <a-list class="w-[240px]" size="small" hoverable>
          <a-list-item class="cursor-pointer" @click="fileSourceClick(null)"
                       :class="searchForm.fileSource === null ? 'file-source-active' : ''">全部文件
          </a-list-item>
          <a-list-item
        ...
        <a-row class="w-full flex-1 mt-3 overflow-y-auto">
          <a-table class="w-[100%]" :scroll="{ minWidth: 600, y: '100%' }" :columns="datatable.columns"
            ...
            <template #fileName="{ record }">
              <a-tooltip :content="record.fileName">
                <div class="w-[190px] flex items-center truncate">
                  <!-- 文件类型图标 -->
                  <img v-if="record.fileType.includes('image')" style="width: 20px"
                       src="../../../assets/file-icon/image.svg" alt="">
            ...
            <template #operation="{ record }">
              <a-button v-perm="['sys:file:download']" type="text" size="mini" @click="downloadBtnClick(record)">
                <template #icon>
                  <icon-download/>
                </template>
              ...
              <a-popconfirm content="确认要删除吗?" @ok="deleteBtnOkClick(record.fileId)">
                <a-button v-perm="['sys:file:del']" type="text" status="danger" size="mini">
                  <template #icon>
                    <icon-delete/>
                  </template>
    ...
    <!-- 文件选择组件演示 -->
    <a-modal v-model:visible="modal.visible" width="70%" :esc-to-close="false" :mask-closable="false" draggable
             :footer="false">
...
import FileSelect from "~/pages/sys/file/FileSelect.vue";
