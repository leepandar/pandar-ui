  ...
  <a-card :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <a-row class="w-full h-full flex flex-col overflow-x-auto overflow-y-hidden">
      ...
      <a-row class="w-full flex justify-between">
        <a-space>
          <!-- 添加 -->
          <a-button v-perm="['sys:role:add']" type="primary" size="small" @click="addBtnClick()">
            <template #icon>
              <icon-plus/>
            </template>
      ...
      <a-row class="w-full flex-1 mt-3 overflow-y-auto">
        <a-table class="w-[100%]" :scroll="{ minWidth: 600, y: '100%' }" :columns="datatable.columns"
          ...
          <template #operation="{ record }">
            <a-button v-perm="['sys:role:edit']" type="text" size="mini" @click="updateBtnClick(record.roleId)">
              <template #icon>
                <icon-edit/>
              </template>
            ...
            <a-popconfirm content="确认要删除吗?" @ok="deleteBtnOkClick(record.roleId)">
              <a-button v-perm="['sys:role:del']" type="text" status="danger" size="mini">
                <template #icon>
                  <icon-delete/>
                </template>
...
import RoleEdit from "~/pages/sys/role/RoleEdit.vue";
import RoleDetail from "~/pages/sys/role/RoleDetail.vue";
