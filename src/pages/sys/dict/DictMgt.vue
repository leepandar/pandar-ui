  ...
  <a-card class="p-0" :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <a-row class="w-full h-full flex flex-col overflow-x-auto overflow-y-hidden">
      ...
      <a-row class="w-full flex justify-between">
        <a-space>
          <!-- 添加 -->
          <a-button v-perm="['sys:dict:add']" type="primary" size="small" @click="addBtnClick()">
            <template #icon>
              <icon-plus/>
            </template>
      ...
      <a-row class="w-full flex-1 mt-3 overflow-y-auto">
        <a-table class="w-[100%]" :scroll="{ y: '100%' }" :columns="datatable.columns" :data="datatable.records"
          ...
          <template #operation="{ record }">
            <a-button v-perm="['sys:dict:edit']" type="text" size="mini" @click="updateBtnClick(record.dictType)">
              <template #icon>
                <icon-edit/>
              </template>
            ...
            <a-popconfirm content="确认要删除吗?" @ok="deleteBtnOkClick(record.dictType)">
              <a-button v-perm="['sys:dict:del']" type="text" status="danger" size="mini">
                <template #icon>
                  <icon-delete/>
                </template>
    ...
    <!-- 添加/修改 -->
    <a-modal v-model:visible="modal.visible" width="60%" :esc-to-close="false" :mask-closable="false" draggable
             :footer="false">
...
import DictEdit from '~/pages/sys/dict/DictEdit.vue'
import DictDetail from '~/pages/sys/dict/DictDetail.vue'
