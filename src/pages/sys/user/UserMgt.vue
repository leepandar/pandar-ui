<template>
  <a-card :body-style="{height: 'calc(100vh - 125px)'}">
    ...
    <div class="w-full h-full flex justify-between">
      <!-- 部门数据 -->
      <a-row class="h-full border-r pr-3" v-perm="['sys:dept:query']">
      ...
      <a-row class="flex flex-1 flex-col pl-3 overflow-x-auto overflow-y-hidden">
        ...
        <a-row class="w-full flex justify-between">
          <a-space>
            <!-- 添加 -->
            <a-button v-perm="['sys:user:add']" type="primary" size="small" @click="addBtnClick()">
              <template #icon>
                <icon-plus/>
              </template>
        ...
        <a-row class="w-full flex-1 mt-3 overflow-y-auto">
          <a-table class="w-[100%]" :scroll="{ minWidth: 600, y: '100%' }" :columns="datatable.columns"
            ...
            <template #operation="{ record }">
              <a-button v-perm="['sys:user:edit']" type="text" size="mini" @click="updateBtnClick(record.userId)">
                <template #icon>
                  <icon-edit/>
                </template>
              ...
              <a-popconfirm content="确认要删除吗?" @ok="deleteBtnOkClick(record.userId)">
                <a-button v-perm="['sys:user:del']" type="text" status="danger" size="mini">
                  <template #icon>
                    <icon-delete/>
                  </template>
...
import UserDetail from "~/pages/sys/user/UserDetail.vue";
